...
app.get('/info', (request, response) => {
	...
	response.send(`<div>Phonebook has info for ${ppl_ct} people<br><br>${Date()}</div>`);
});
...
app.post('/api/persons', (request, response) => {
	...
	if (!body.name || !body.number) {
		...
		return response.status(400).json({ error: 'name or number missing from request' })
	};
	const existing_names = people.map(entry => entry.name);
	if (existing_names.includes(body.name)) {
		return response.status(404).json({ error: `name must be unique, ${body.name} already in phonebook` })
	};
