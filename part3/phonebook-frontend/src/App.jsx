...
const PersonEntry = ({ name, number, delete_handler }) => (
  <div>
    {name} {number}
...
const DisplayPersons = ({ persons, search_filter, delete_handler }) => {
  return (
    <div>
      {persons
        .filter((person) =>
          person.name.toUpperCase().includes(search_filter.toUpperCase())
        )
        .map((person) => {
          ...
          return (
            <PersonEntry
              key={person.name}
              delete_handler={() => delete_handler({ ...person })}
              {...person}
            />
          );
        })}
...
const App = () => {
  ...
  const AddPerson = (event) => {
    ...
    const dupicate_name = persons.map((person) => person.name).includes(
      newName,
    );
    if (dupicate_name) {
      if (
        ...
        const current_person = persons.filter((person) =>
          person.name === newName
        )[0];
        ...
        entryservice
          ...
          .then((update_return) =>
            setPersons(persons.map((person) =>
              person.id === update_id ? update_return : person
            ))
          )
          .then((_) => {
            setSuccessMessage(
              `Number for ${current_person.name} has been updated!`,
            );
            setTimeout(() => {
          ...
          .catch((error) => {
            setErrorMessage(
              `Could not locate entry for ${current_person.name} to update on server.`,
            );
            setTimeout(() => {
